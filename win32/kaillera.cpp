...
static const uint32 k_payload_sync_mask  = k_payload_sync_sram | k_payload_sync_state |
                                           k_payload_sync_clear | k_payload_sync_reset;
...
enum CtrlOp : uint8 { CTRL_NONE=0, CTRL_FLAGS=1, CTRL_SRAM=2, CTRL_STATE=3 };
enum StartupFlag : uint8 { SF_SRAM=0x01, SF_STATE=0x02, SF_CLEARRAM=0x04 };
enum ChatSramPhase : uint8 { CHAT_SRAM_NONE=0, CHAT_SRAM_BEGIN=1, CHAT_SRAM_CHUNK=2, CHAT_SRAM_END=3 };
...
struct KailleraApi
{
    ...
    KailleraApi() : dll(NULL), getVersion(NULL), init(NULL), shutdown(NULL),
        setInfos(NULL), selectServerDialog(NULL), modifyPlayValues(NULL),
        chatSend(NULL), endGame(NULL), initialised(false) {}
};
...
struct KailleraRuntime
{
    ...
    KailleraRuntime()
        : active(false), suppress_emulation(false), startup_sync(false),
          ...
          runtime_sync_player(0), num_players(0), session_player_count(0), pending_auto_movie(0),
          wait_frames(0), announce_frames(0), name_sent(false), flags_sent(false),
          flags_known(false), hacks_known(false), received_remote_sram(false), auto_movie_owned(false),
          startup_flags(0), remote_sync_flags(0), remote_startup_reset(false), startup_release_pending(false),
          runtime_release_pending(false), outgoing_offset(0), outgoing_opcode(CTRL_NONE),
          outgoing_id(0), in_sram_received(0), in_state_received(0),
          in_sram_total(0), in_state_total(0), in_sram_id(0), in_state_id(0),
          transfer_serial(0), cache_ready(false),
          ...
          pv_state_ready(false),
          chat_input_active(false), chat_input_swallow_until_tick(0)
    {
        memset(player_names,      0, sizeof(player_names));
        memset(player_name_known, 0, sizeof(player_name_known));
...
static void update_state_transfer_title(bool sending, size_t completed, size_t total);
...
{
    size_t a = 0, b = s.size();
    while (a < b && (s[a] <= 32 || s[a] == 0)) ++a;
    while (b > a && (s[b-1] <= 32 || s[b-1] == 0)) --b;
...
static std::wstring to_wide(const std::string &u8)
{
    ...
    if (n > 1) MultiByteToWideChar(CP_UTF8, 0, u8.c_str(), -1, &out[0], n);
    return out;
}
...
static std::string make_unique_movie_path(void)
{
    ...
    if (stem[0] == '\0') strcpy(stem, "kaillera");
...
{
    if (!KailleraConfig.AutoRecordMovie) return;
    stop_auto_movie();
    ...
    int result = S9xMovieCreate(path.c_str(), controllers, opts,
                                meta.c_str(), (int)meta.size());
    if (result == SUCCESS)
    {
    ...
    {
        post_status("Kaillera: auto movie start failed (%d) romfn=%s path=%s errno=%d",
...
static std::vector<uint8> compress_data(const uint8 *raw, uint32 raw_size)
{
    ...
    if (compress2(&buf[4], &out_size, raw, raw_size,
...
static inline std::vector<uint8> compress_state(const uint8 *raw, uint32 raw_size)
{
...
static std::vector<uint8> read_file_bytes(const char *path)
{
    ...
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    if (sz <= 0 || sz > 64 * 1024 * 1024) { fclose(f); return result; }
    fseek(f, 0, SEEK_SET);
    result.resize((size_t)sz);
    size_t got = fread(&result[0], 1, (size_t)sz, f);
    fclose(f);
    if (got != (size_t)sz) result.clear();
    return result;
}
...
/* Decompress gzip data in memory.  Returns decompressed bytes, or empty on failure. */
...
static void apply_state_from_pv_transfer(const std::vector<uint8> &data)
{
    ...
    if (is_gzip(data)) {
        ...
        if (raw_size == 0 || raw_size > 64u * 1024u * 1024u) return;
        std::vector<uint8> raw(raw_size);
        uLong rl = raw_size;
        if (uncompress(&raw[0], &rl, &data[4],
                        (uLong)(data.size() - 4)) != Z_OK) {
...
static void do_playvalues_padding(int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (ret < 0) { rt.active = false; return; }
        MSG msg;
 ...
 *   30 zero-padding → 1 size frame (0x05) → 30 padding → N data (0x06) → 30 padding
 *
 ...
 */
static std::vector<std::vector<uint8>> do_playvalues_transfer(
    const std::vector<uint8> &sender_data, int numplayers,
    bool show_pct = false, bool is_sending = true)
{
    ...
    if (ret > 0) {
        for (int p = 0; p < numplayers && p < 8; p++) {
            if (frame[p * k_payload_size] == 5) {
                ...
                if (psz > sync_size) sync_size = psz;
            }
        }
    }
    ...
    for (uint32 chunk = 0; chunk < num_chunks; chunk++) {
        ...
        if (offset < local_size) {
            frame[0] = 6;
            for (int b = 0; b < 4; b++)
                frame[1+b] = (offset + b < local_size)
                             ? sender_data[offset + b] : 0;
        }
        ret = api.modifyPlayValues(frame, k_payload_size);
        if (ret < 0) { rt.active = false; return recv_data; }
        ...
        if (show_pct && num_chunks > 100 &&
            ...
            update_state_transfer_title(is_sending,
                                        (size_t)(chunk * 4), (size_t)sync_size);
            show_status(is_sending ? "Kaillera: sending state %d%%"
                                   : "Kaillera: receiving state %d%%", pct);
        }
    }
...
{
    if (rt.incoming_state.size() < 4) return;
    uint32 raw_size = READ_DWORD(&rt.incoming_state[0]);
    std::vector<uint8> raw(raw_size);
    uLong rl = raw_size;
    if (uncompress(&raw[0], &rl,
                   &rt.incoming_state[4],
...
{
    if (rt.incoming_sram.size() < 4) return;
    uint32 raw_size = READ_DWORD(&rt.incoming_sram[0]);
    if (raw_size == 0 || raw_size > 0x80000) return;
    std::vector<uint8> raw(raw_size);
    uLong rl = raw_size;
    if (uncompress(&raw[0], &rl,
                   &rt.incoming_sram[4],
...
{
    if (!api.chatSend || payload.empty()) return;
...
static KailleraHackSettings capture_hack_settings(void)
{
    ...
    hacks.block_invalid_vram_access_master = Settings.BlockInvalidVRAMAccessMaster;
...
static void apply_hack_settings(const KailleraHackSettings &hacks)
{
    ...
    Settings.BlockInvalidVRAMAccessMaster = hacks.block_invalid_vram_access_master;
...
static uint32 build_playdata_sync_flags(void)
{
    if (rt.startup_sync)
    {
        ...
        if (rt.startup_flags & SF_SRAM)
            return k_payload_sync_sram | (rt.startup_release_pending ? k_payload_sync_reset : 0);
        if (rt.startup_flags & SF_CLEARRAM)
            return k_payload_sync_clear | (rt.startup_release_pending ? k_payload_sync_reset : 0);
        return 0;
    }
...
static bool begin_chat_transfer(std::vector<uint8> data, bool startup_sync, bool is_state)
{
    ...
    else if (rt.local_player < 1 || rt.local_player > 8 ||
             !rt.authoritative)
        return false;
...
static bool can_receive_chat_transfer(int sender, bool startup_sync, bool is_state)
{
...
{
    if (rt.local_player < 1 || rt.local_player > 8) return;
...
static void handle_chat_sram_ack(int player, uint32 id, uint8 phase, uint32 chunk_index)
{
    if (!rt.chat_sram_active || !rt.chat_sram_sender) return;
    if (id != rt.chat_sram_id || phase != rt.chat_sram_phase) return;
    if (phase == CHAT_SRAM_CHUNK && chunk_index != current_chat_sram_chunk_index()) return;
    if (player < 1 || player > 8 || player == rt.local_player) return;
    uint8 bit = (uint8)(1u << (player - 1));
    if (!(rt.chat_sram_expected_acks & bit)) return;
    rt.chat_sram_received_acks |= bit;
    if ((rt.chat_sram_received_acks & rt.chat_sram_expected_acks) != rt.chat_sram_expected_acks)
        return;
    rt.chat_sram_received_acks = 0;
    rt.chat_sram_last_send_tick = 0;
    if (rt.chat_sram_phase == CHAT_SRAM_BEGIN)
    {
        rt.chat_sram_phase = rt.chat_sram_data.empty() ? CHAT_SRAM_END : CHAT_SRAM_CHUNK;
        return;
    }
...
{
    if (!rt.chat_sram_active) return;
    if (!rt.chat_sram_sender)
    {
        if (rt.chat_sram_is_state && rt.in_state_total > 0)
        {
            update_state_transfer_title(false, rt.in_state_received, rt.in_state_total);
    ...
    show_status(rt.chat_sram_is_state ? KAILLERA_SENDING_STATE : KAILLERA_SENDING_SRAM);
 ...
 * -------------------------------------------------------------------------- */
static inline bool name_byte_ok(unsigned char c)
{
    return c >= 32 && c != '\t' && c != '\r' && c != '\n';
}
...
{
    int s = 0;
    if (buf[28] + (buf[29]<<8) + buf[30] + (buf[31]<<8) == 0xFFFF) s += 3;
    if (buf[26] == 0x33) s += 2;
    if ((buf[21] & 0xF) < 4)  s += 2;
    if (!(buf[61] & 0x80))    s -= 4;
    if ((1 << (buf[23] - 7)) > 48) s -= 1;
    if (buf[25] < 14)         s += 1;
    if (!all_ascii(buf, 20))  s -= 1;
    return s;
}

static int detect_header(const std::vector<uint8> &d)
{
    if (d.size() < 0x8000) return 0;
    int has = 0, none = 0;
    int remain = (int)d.size() & 0x7FFF;
    if (remain == 0)    none += 3;
    else if (remain == 512) has  += 2;
    if (d.size() >= 512)
    {
        unsigned short s = 0;
        for (int i = 0; i < 512; i++) s += d[i];
        if (s < 2500) has += 2;
        if (d[8]==0xAA && d[9]==0xBB && d[10]==4) has += 3;
        else if (memcmp(&d[0], "GAME DOCTOR SF 3", 16) == 0) has += 5;
                                  ...
                                  std::string       &game_key)
{
    ...
    if (usable >= 0x500000)
    {
        ...
        { hbase = (size_t)hdr_sz + 0x40FFB0; found = true; }
    }
    if (!found && usable >= 0x10000)
    {
        ...
        if (hi + 0x40 <= d.size())
        {
            ...
            hbase = (ls > hs) ? (size_t)hdr_sz + 0x7FB0
                              : (size_t)hdr_sz + 0xFFB0;
            found = true;
            if (usable >= 0x20000)
            {
    ...
    if (!found || hbase + 0x2C >= d.size()) return false;
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "%s (%s) v1.%d [%08X]",
             name.c_str(), region_code(region), revision, (uint32)crc);
    game_key = buf;
    return true;
}
...
static void load_valid_exts(void)
{
    ...
    {
        for (auto *e : {"smc","sfc","swc","fig","bs","st","zip","gz","jma","msu1"})
                            ...
                            std::vector<FileCacheEntry> &out)
{
    ...
    do
    {
        if (!lstrcmp(fd.cFileName, TEXT(".")) ||
            !lstrcmp(fd.cFileName, TEXT(".."))) continue;
        ...
        else
        {
            ...
            if (!is_supported(u8rel)) continue;
...
static bool load_file_cache(std::vector<FileCacheEntry> &out)
{
    ...
    std::string hdr;
    std::getline(f, hdr);
    if (hdr != k_file_cache_hdr) return false;
    std::string line;
    while (std::getline(f, line))
    {
        size_t a = line.find('\t');
...
static bool load_game_cache(const std::vector<FileCacheEntry> &current,
                             std::vector<GameCacheEntry>       &out)
{
    ...
    std::string hdr;
    std::getline(f, hdr);
    if (hdr != k_game_cache_hdr) return false;

    /* Build a quick lookup for the current file list */
    ...
    while (std::getline(f, line))
    {
        ...
        auto it = cur_map.find(g.relative_path);
        if (it == cur_map.end()) return false; /* stale cache */
        g.size       = it->second->size;
        g.write_time = it->second->write_time;
        out.push_back(g);
    }
...
static void rebuild_game_lookup(void)
{
    ...
    /* Add special non-startable entries at the top of the list */
    ...
    /* Sort game list alphabetically by display key (case-insensitive) */
    ...
    rt.infos.gameList = rt.game_list_blob.empty()
                            ? const_cast<char*>("\0")
                            : &rt.game_list_blob[0];
}
...
static void ensure_game_cache(void)
{
    ...
    bool same = (load_file_cache(cached_files) && cached_files == current);
    bool ok   = same && load_game_cache(current, cached_games);

    if (ok)
    {
        rt.file_cache = cached_files;
        rt.game_cache = cached_games;
    }
    else
    {
        /* 3. Rescan ROM metadata */
        rt.file_cache = current;
        std::set<std::string> used_keys;
        for (auto &fe : current)
        {
            std::string full = S9xGetDirectory(ROM_DIR) + "\\" + fe.relative_path;
            std::string key;
            if (!extract_rom_metadata(full, key)) continue;
            /* Deduplicate */
            ...
            GameCacheEntry g;
            g.key           = unique;
            g.relative_path = fe.relative_path;
            g.size          = fe.size;
            g.write_time    = fe.write_time;
            rt.game_cache.push_back(g);
        }
...
static void consume_player_payload(int pidx, const uint8 *payload)
{
    ...
    if ((sync_flags & SF_STATE) && player != rt.local_player &&
        player == 1)
    {
...
{
    for (int i = 0; i < rt.num_players && i < 8; i++)
        if (!rt.player_name_known[i]) return false;
    return true;
}
...
{
    if (!rt.startup_sync) return;
    
    if (rt.startup_flags & SF_STATE)
    {
        if (rt.authoritative)
        {
            if (rt.chat_sram_active && rt.chat_sram_startup_sync && rt.chat_sram_is_state)
                return;
            ...
            if (rt.remote_sync_flags & SF_STATE) return;
        }
        else
        {
            if (rt.chat_sram_active && rt.chat_sram_startup_sync && rt.chat_sram_is_state)
                return;
            if (rt.remote_sync_flags & SF_STATE) return;
            if (rt.incoming_state.empty() ||
                rt.in_state_received < rt.in_state_total) return;
...
{
    if (!rt.runtime_state_sync) return;
    ...
    auto received = do_playvalues_transfer(
        is_sender ? rt.pending_pv_state : std::vector<uint8>(),
        rt.num_players,
        true, is_sender);
...
static void WINAPI chat_callback(char *nick, char *text)
{
    ...
    if (*(const uint32*)text == k_da_sentinel)
    {
        ...
        if (!data.empty() && data[0] == k_cmd_sram_begin && data.size() >= 10)
        {
            ...
            {
                if (rt.chat_sram_phase == CHAT_SRAM_END && !rt.chat_sram_active && id == rt.chat_sram_id)
                {
                    send_chat_sram_ack(id, CHAT_SRAM_BEGIN, 0);
                    return;
                }
                if (rt.chat_sram_active && rt.chat_sram_phase != CHAT_SRAM_NONE && id != rt.chat_sram_id)
                    return;
                if (total > 0 && total <= (is_state ? max_chat_state_size() : 0x100000))
                {
        ...
        if (!data.empty() && data[0] == k_cmd_sram_chunk && data.size() >= 15)
        {
            ...
            if (rt.chat_sram_active &&
                can_receive_chat_transfer(sender, rt.chat_sram_startup_sync, rt.chat_sram_is_state))
            {
                uint32 id = READ_DWORD(&data[2]);
                ...
                std::vector<uint8> &incoming = rt.chat_sram_is_state ? rt.incoming_state
                                                                     : rt.incoming_sram;
                uint32 active_id = rt.chat_sram_is_state ? rt.in_state_id : rt.in_sram_id;
                size_t &received = rt.chat_sram_is_state ? rt.in_state_received
                                                         : rt.in_sram_received;
                if (id == active_id && offset == expected_offset &&
                    offset < incoming.size() &&
                    offset + chunk <= incoming.size())
                {
        ...
        if (!data.empty() && data[0] == k_cmd_sram_end && data.size() >= 6)
        {
            int sender = data[1];
            uint32 id = READ_DWORD(&data[2]);
            if (can_receive_chat_transfer(sender, rt.chat_sram_startup_sync, rt.chat_sram_is_state))
            {
                if (!rt.chat_sram_active && rt.chat_sram_phase == CHAT_SRAM_END && id == rt.chat_sram_id)
                {
                    send_chat_sram_ack(id, CHAT_SRAM_END, 0);
                    return;
                }
                size_t received = rt.chat_sram_is_state ? rt.in_state_received : rt.in_sram_received;
                size_t total = rt.chat_sram_is_state ? rt.incoming_state.size() : rt.incoming_sram.size();
                uint32 active_id = rt.chat_sram_is_state ? rt.in_state_id : rt.in_sram_id;
...
static void WINAPI drop_callback(char *nick, int playernb)
{
    ...
    {
        snprintf(rt.player_names[playernb-1], k_name_max+1,
                 "%s (dropped)", nick ? nick : "?");
...
static void WINAPI infos_callback(char *game)
{
    ...
    if (it == rt.game_lookup.end()) return;

    const GameCacheEntry &e = rt.game_cache[it->second];
    char msg[1024];
    snprintf(msg, sizeof(msg),
             "%s\n\nFile: %s\nSize: %llu bytes",
             game, e.relative_path.c_str(),
...
static void run_kaillera_game_loop(void)
{
    while (rt.active)
    {
        ...
        /* After ProcessInputs returns: start SMV (not inside Kaillera DLL call chain). */
...
static std::string ExtractNickFromIni(const std::string& path)
{
    ...
    while (std::getline(file, line))
    {
        ...
        for (size_t i = 0; i < lower_line.size(); ++i) lower_line[i] = (char)tolower((unsigned char)lower_line[i]);
        ...
        if (pos != std::string::npos)
        {
            ...
            if (end != std::string::npos) nick = nick.substr(0, end + 1);
...
static int WINAPI game_callback(char *game, int player, int numplayers)
{
    ...
    if (game && (strcmp(game, k_away_entry) == 0 || strcmp(game, k_chat_entry) == 0))
        return 0;

    auto it = rt.game_lookup.find(game ? game : "");
    if (it == rt.game_lookup.end())
    {
        MessageBoxA(GUI.hWnd, "Selected game not found in cache.",
    ...
    int kaillera_controller_option = controller_option_for_player_count(numplayers);
    ...
    if (numplayers > 1)
    {
        ...
        if (rt.authoritative && KailleraConfig.TransferSRAM && get_game_sram_size() > 0)
        ...
        KailleraHackSettings hacks;
        if (rt.authoritative) hacks = capture_hack_settings();
        else memset(&hacks, 0, sizeof(hacks));
        ...
        // Extract local nickname and bundle directly into the startup sequence payload natively
        std::string nick = ExtractKailleraNick();
        uint8 nlen = (uint8)nick.length();
        if (nlen > k_name_max) nlen = k_name_max;
        ...
        while (startup_data.size() % 4 != 0) startup_data.push_back(0);
        for (int pad = 0; pad < 4; pad++) startup_data.push_back(0);

        /* Extra initial padding for startup (beyond the 30 inside do_playvalues_transfer) */
        do_playvalues_padding(30);
        ...
        for (int p = 0; p < numplayers; p++) {
            if (recv_data[p].size() >= 36) {
                ...
                if (hacks_valid && p == 0 && rt.startup_sync && !rt.authoritative) {
                    ...
                    hs.superfx_clock_multiplier = READ_DWORD(&recv_data[p][offset]); offset += 4;
                    hs.interpolation_method = (int32)READ_DWORD(&recv_data[p][offset]); offset += 4;
                    hs.overclock_mode = (int32)READ_DWORD(&recv_data[p][offset]); offset += 4;
                    hs.one_clock_cycle = (int32)READ_DWORD(&recv_data[p][offset]); offset += 4;
                    hs.one_slow_clock_cycle = (int32)READ_DWORD(&recv_data[p][offset]); offset += 4;
                    hs.two_clock_cycles = (int32)READ_DWORD(&recv_data[p][offset]); offset += 4;
                    hs.max_sprite_tiles_per_line = (int32)READ_DWORD(&recv_data[p][offset]); offset += 4;
                    apply_hack_settings(hs);
                    rt.hacks_known = true;
                    offset = hack_base + 34; // Ensure perfectly aligned advancement past hack padding block
                } else {
                ...
                if (p == 0 && rt.startup_sync && !rt.authoritative) {
                    if (rt.startup_flags == SF_CLEARRAM) {
                        ...
                        if (rem_len >= 8) {
                            ...
                            offset += 8;
                            if (raw_size > 0 && raw_size <= 0x80000 && zlib_len <= rem_len - 8) {
                                size_t game_sz = get_game_sram_size();
                                ...
                                if (copy_sz) {
                                    ...
                                    uLong rl = raw_size;
                                    if (uncompress(&raw[0], &rl,
                                                   &recv_data[p][offset], (uLong)zlib_len) == Z_OK) {
                                        memcpy(Memory.SRAM, &raw[0], std::min((size_t)rl, copy_sz));
                                        post_status("Kaillera: received %u bytes of SRAM from P1.", (uint32)copy_sz);
                                    } else {
                                        post_status("Kaillera: SRAM zlib decompress failed.");
                                    }
                                }
                                if (copy_sz < game_sz) memset(Memory.SRAM + copy_sz, 0, game_sz - copy_sz);
                                CPU.SRAMModified = FALSE;
                                rt.received_remote_sram = true;
                            }
                        }
        ...
        fill_missing_names();
        if (rt.authoritative && rt.startup_flags == SF_CLEARRAM && rt.startup_sync) {
            Memory.ClearSRAM(FALSE);
            CPU.SRAMModified = FALSE;
        }
        rt.startup_sync = false;
...
#undef RESOLVE
    ...
    if (!api.init || !api.shutdown || !api.setInfos ||
        !api.selectServerDialog || !api.modifyPlayValues ||
        !api.chatSend || !api.endGame)
    {
        FreeLibrary(api.dll);
...
{
    if (!load_dll()) { rt.cache_ready = false; return false; }
...
void S9xKailleraShowClient(HWND parent)
{
    if (!S9xKailleraInit())
    {
        MessageBox(parent ? parent : GUI.hWnd,
                   TEXT("Could not load kailleraclient.dll."),
                   TEXT("Kaillera"),
                   MB_OK | MB_ICONERROR);
        return;
    }
...
bool S9xKailleraSuppressEmulation(void) { return rt.active && rt.suppress_emulation; }
...
{
    return rt.received_remote_sram;
}
...
{
    if (!rt.active || !api.modifyPlayValues) return;
    ...
    if (players > 0) rt.num_players = players;

    /* Zero out all pads, then fill from received payloads */
    for (int i = 0; i < count; i++) joypads_io[i] = 0;
...
void S9xKailleraNotifySramSaved(void)
{
    ...
    size_t sram_sz = get_game_sram_size();
    if (!sram_sz) return; /* game has no SRAM — nothing to send */
...
bool S9xKailleraHandleKeyboardMessage(UINT msg, WPARAM wParam, LPARAM /*lParam*/)
{
    ...
    case WM_SYSKEYDOWN:
        if (wParam == VK_RETURN)
        {
            ...
            close_chat_input();
            if (!message.empty() && api.chatSend && api.chatSend(const_cast<char *>(message.c_str())) < 0)
...
LRESULT S9xKailleraHandleUiMessage(UINT msg, WPARAM /*wParam*/, LPARAM lParam)
{
    if (msg == WM_KAILLERA_STATUS || msg == WM_KAILLERA_CHAT)
    {
        ...
        if (text) { S9xSetInfoString(text); free(text); }
        return 0;
    }
    return 0;
}
                                    ...
                                    WPARAM wParam, LPARAM /*lParam*/)
{
    ...
    case WM_INITDIALOG:
        WinRefreshDisplay();
        SendDlgItemMessage(hDlg, IDC_KAILLERA_TRANSFER_SRAM, BM_SETCHECK,
            KailleraConfig.TransferSRAM    ? BST_CHECKED : BST_UNCHECKED, 0);
        SendDlgItemMessage(hDlg, IDC_KAILLERA_CHAT_OSD, BM_SETCHECK,
            KailleraConfig.ShowChatInOSD   ? BST_CHECKED : BST_UNCHECKED, 0);
        SendDlgItemMessage(hDlg, IDC_KAILLERA_AUTO_MOVIE, BM_SETCHECK,
            KailleraConfig.AutoRecordMovie   ? BST_CHECKED : BST_UNCHECKED, 0);
        SendDlgItemMessage(hDlg, IDC_KAILLERA_STOP_MOVIE, BM_SETCHECK,
            KailleraConfig.AutoStopMovieOnEnd ? BST_CHECKED : BST_UNCHECKED, 0);
        SendDlgItemMessage(hDlg, IDC_KAILLERA_NEVER_BLOCK_SRAM, BM_SETCHECK,
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            KailleraConfig.NeverBlockSRAMSave =
                IsDlgButtonChecked(hDlg, IDC_KAILLERA_NEVER_BLOCK_SRAM) == BST_CHECKED;
            WinSaveConfigFile();
            EndDialog(hDlg, 1);
            return TRUE;
        case IDCANCEL:
